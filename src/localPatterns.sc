patterns:
    ...
    $yes = * (могу/смогу/верно/да/lf/дада/ok/так точно/ага/угу*/конечно/согласен/хорошо/давай) *
    $dontknow = * ([еще/пока] не (выбрал/решил/знаю/помню/определился)/{нет [пока/еще]}/потом [решу]) *
    $no = * (не могу/неверно/неправильно/не*(верно/правильно)/иначе/неа/ноу/другой/нет/ytn) *
    $refuse = * (не (назову/скажу/нужен/чего/хочу)/ни (чего/~какой/как)/никак/никакой/уйди/отстань/отвяжись) *
